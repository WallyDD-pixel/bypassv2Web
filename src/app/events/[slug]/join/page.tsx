  ...
  return (
    <>
      ...
      <Dialog open={showGratuitInfo} onClose={() => setShowGratuitInfo(false)} className="fixed z-50 inset-0 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full mx-auto shadow-xl z-10">
          <Dialog.Title className="text-lg font-bold mb-2 text-slate-900 dark:text-white">Pourquoi c'est gratuit pour les filles ?</Dialog.Title>
          <Dialog.Description className="text-slate-700 dark:text-slate-300 mb-4">
          ...
          <button onClick={() => setShowGratuitInfo(false)} className="mt-2 px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 font-semibold">Fermer</button>
        </div>
      </Dialog>
      <main className="max-w-6xl mx-auto px-6 py-14">
        ...
        <div className="mb-10 flex items-center gap-3">
          <h1 className="hidden md:block text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Rejoindre un groupe</h1>
        </div>
        ...
        <div className="grid md:grid-cols-[1fr_370px] gap-12 items-start min-h-[60vh]">
          <div className="w-full">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {allGroups.map((g, idx) => {
                    ...
                    return (
                      <div
                        key={g.name || idx}
                        className="group w-full rounded-2xl border border-black/10 dark:border-white/15 bg-white/90 dark:bg-slate-900 shadow hover:shadow-xl transition overflow-hidden flex flex-col items-center p-6 relative"
                      >
                        ...
                        <div className="font-bold text-slate-900 dark:text-white mb-1 truncate w-full text-center text-lg">{g.name}</div>
                        <div className="flex flex-wrap gap-2 justify-center text-xs text-slate-700 dark:text-slate-200 mb-2">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-200"><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" stroke="#db2777" strokeWidth="1.5"/><path d="M7 21c.8-2.7 3.2-4.5 5-4.5S16.2 18.3 17 21" stroke="#db2777" strokeWidth="1.5" strokeLinecap="round"/></svg> {g.femaleCount}</span>
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-200"><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" stroke="#2563eb" strokeWidth="1.5"/><path d="M15 21v-3a3 3 0 1 0-6 0v-3" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round"/></svg> {g.maleCount}</span>
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-700/40 text-slate-700 dark:text-slate-200">{g.members} membres</span>
                        </div>
                        {/* Prix mis en avant */}
                        ...
                        {g.arrivalTime && (
                          <div className="flex items-center justify-center gap-1 text-xs text-slate-700 dark:text-slate-200 mb-2">
                        ...
                        <button
                          ...
                          className={`mt-4 w-full rounded-xl font-semibold py-2 px-4 text-base shadow transition
                            ${alreadyRequested
                              ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-200 cursor-not-allowed opacity-70'
            ...
            {mode === "wizard" && activeGroup && (
                ...
                {wizardStep === 0 && (() => {
                  ...
                  return (
                    <div>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="mb-2">
                            {maleSpots0 > 0 ? (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[12px] bg-white text-slate-900 dark:bg-white dark:text-slate-900 border border-emerald-500/30">
                              ...
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[12px] bg-white text-slate-900 dark:bg-white dark:text-slate-900 border border-slate-500/30">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    ...
                    <div className="mb-3 flex items-center gap-2 text-[12px]">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-black/10 dark:border-white/15 bg-white/70 dark:bg-white/10 text-slate-700 dark:text-slate-300">
                    ...
                    {avatars.length === 0 ? (
                      <div className="text-sm text-slate-600 dark:text-slate-400">Aperçu non disponible.</div>
                    ) : (
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                        {avatars.slice(0, 15).map((src, idx) => {
                          ...
                          return (
                            <div key={idx} className="relative aspect-square rounded-2xl border border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/10 overflow-hidden shadow-sm">
                              {/* Badge genre */}
                              <span className={`absolute top-1 left-1 inline-flex items-center justify-center h-6 w-6 rounded-full border border-white/70 dark:border-black/40 shadow ${isFemale ? 'bg-pink-600/80' : 'bg-blue-600/80'}`} title={isFemale ? 'Fille' : 'Garçon'}>
                                {isFemale ? (
              ...
              {wizardStep === 1 && (() => {
                ...
                return (
                  <div>
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-2">Prix par homme</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
                      Prix fixé par le groupe: <strong>{fmt(basePrice)}</strong>. Vous pouvez payer ce prix ou ajouter un extra.
                    </p>
                    <div className="grid place-items-center">
                      <div className="text-4xl font-bold tracking-wide tabular-nums text-slate-900 dark:text-white mb-3">{fmt(selectedAmount)}</div>
                      <div className="inline-flex items-center gap-2 rounded-2xl border border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/10 backdrop-blur px-2 py-2">
                        <button
                          ...
                          className={`h-10 px-3 grid place-items-center rounded-xl border border-black/10 dark:border-white/10 text-sm font-semibold select-none active:scale-95 transition
                            ${!canMinus5 ? 'bg-slate-200 text-black cursor-not-allowed' : 'bg-white/80 dark:bg-white/10 text-slate-900 dark:text-white'}`}
                          disabled={!canMinus5}
                        >
                          −5
                        </button>
                        <button
                          ...
                          className={`h-10 px-3 grid place-items-center rounded-xl border border-black/10 dark:border-white/10 text-sm font-semibold select-none active:scale-95 transition
                            ${!canMinus1 ? 'bg-slate-200 text-black cursor-not-allowed' : 'bg-white/80 dark:bg-white/10 text-slate-900 dark:text-white'}`}
                          disabled={!canMinus1}
                        >
                          −1
                        </button>
                        <button type="button" aria-label="+1 €" onClick={() => setSelectedAmount((v) => Math.round((v + 1) * 100) / 100)} className="h-10 px-3 grid place-items-center rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 border border-black/10 dark:border-white/10 text-sm font-semibold select-none active:scale-95 transition">+1</button>
                        <button type="button" aria-label="+5 €" onClick={() => setSelectedAmount((v) => Math.round((v + 5) * 100) / 100)} className="h-10 px-3 grid place-items-center rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 border border-black/10 dark:border-white/10 text-sm font-semibold select-none active:scale-95 transition">+5</button>
                      </div>
                      <div className="mt-3 flex flex-wrap justify-center gap-2">
                        {[basePrice, basePrice + 5, basePrice + 10].map((n, idx) => (
                          <button key={idx} type="button" onClick={() => setSelectedAmount(Math.max(basePrice, n))} className={`px-3 py-1.5 rounded-full text-sm border transition ${selectedAmount === Math.max(basePrice, n) ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 border-black/10 dark:border-white/10' : 'bg-white/60 dark:bg-white/10 text-slate-700 dark:text-slate-300 border-black/10 dark:border-white/10 hover:bg-white'}`}>{fmt(Math.max(basePrice, n))}</button>
                        ))}
                      </div>
                    </div>

                    <div className="mt-5 flex items-center justify-between gap-3">
                      <button onClick={() => setWizardStep(0)} className="rounded-xl px-4 py-2 text-sm font-semibold bg-slate-200 text-slate-900 dark:bg-white/10 dark:text-white">Retour</button>
                      <button onClick={() => setWizardStep(2)} className="rounded-xl px-4 py-2 text-sm font-semibold bg-slate-900 text-white dark:bg-white dark:text-slate-900">Suivant</button>
                    </div>
                  </div>
                );
              })()}
                ...
                return (
                  <div>
                    <div className="rounded-xl border border-black/10 dark:border-white/15 bg-white/70 dark:bg-white/10 p-4 space-y-2 text-sm text-slate-700 dark:text-slate-300">
                      <div className="flex items-center gap-2"><span className="font-semibold text-slate-900 dark:text-white">Votre profil:</span> Homme (détecté automatiquement)</div>
                      ...
                      <div className="flex items-center gap-2"><span className="font-semibold text-slate-900 dark:text-white">Montant que vous paierez:</span> {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(Math.max(0, selectedAmount))}</div>
                    ...
                    <div className="mt-4">
                      <div className="text-sm font-semibold text-slate-900 dark:text-white mb-2">Voici les personnes déjà dans le groupe</div>
                      {(() => {
                        const avatars = (activeGroup.memberAvatars && activeGroup.memberAvatars.length > 0)
                          ? activeGroup.memberAvatars
                          : (activeGroup.avatarUrl ? [activeGroup.avatarUrl] : []);
                        ...
                        return (
                          <div className="grid grid-cols-5 sm:grid-cols-6 gap-3">
                            {avatars.slice(0, 12).map((src, idx) => (
                              <div key={idx} className="aspect-square rounded-xl border border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/10 overflow-hidden shadow-sm">
                                {/* flou léger */}
                      ...
                      <div className="mt-2 text-[12px] text-slate-500 dark:text-slate-400">Les photos sont volontairement floutées pour préserver la confidentialité.</div>
                    </div>

                    {/* Encart explicatif bienveillant */}
                    <div className="mt-4 rounded-xl border border-amber-300/60 dark:border-amber-400/30 bg-amber-50/80 dark:bg-amber-500/10 p-4">
                    ...
                    <div className="mt-5 flex items-center justify-between gap-3">
                      <button onClick={() => setWizardStep(1)} className="rounded-xl px-4 py-2 text-sm font-semibold bg-slate-200 text-slate-900 dark:bg-white/10 dark:text-white">Retour</button>
                      <button
          ...
          <aside className="hidden md:block rounded-2xl border border-black/10 dark:border-white/15 bg-white/80 dark:bg-white/5 backdrop-blur-xl p-8 shadow-[0_12px_40px_rgba(0,0,0,0.14)] sticky top-24 min-w-[370px] max-w-sm ml-8">
            <div className="flex items-start gap-3">
              <span className="mt-0.5 h-8 w-8 rounded-full bg-slate-900 text-white dark:bg-white dark:text-slate-900 grid place-items-center shadow">
                {/* megaphone icon */}
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              ...
              <div>
                <div className="text-sm font-extrabold text-slate-900 dark:text-white">Proclamation</div>
                <ul className="mt-2 space-y-2 text-[13px] text-slate-700 dark:text-slate-300 list-disc pl-4">

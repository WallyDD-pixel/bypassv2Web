  ...
  if (!pending) {
    return (
      <main className="w-full max-w-md mx-auto px-4 py-16">
        <div className="rounded-2xl border border-black/10 dark:border-white/15 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow">
        ...
        <form onSubmit={submit} className="space-y-5">
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setGender("male")} className={`rounded-xl px-4 py-3 font-semibold border ${gender === "male" ? "border-slate-900 bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/5"}`}>Homme</button>
              <button type="button" onClick={() => setGender("female")} className={`rounded-xl px-4 py-3 font-semibold border ${gender === "female" ? "border-slate-900 bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/5"}`}>Femme</button>
            </div>
          </fieldset>
          ...
          <div>
            ...
            <label onClick={() => fileInputRef.current?.click()} className="block rounded-2xl border border-black/10 dark:border-white/15 bg-white/60 dark:bg-white/5 backdrop-blur-xl p-4 cursor-pointer">
              {avatarUrl ? (

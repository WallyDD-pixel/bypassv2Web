...
export default function EventDetailPage() {
  ...
  return (
    ...
    <div className="mb-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/60 backdrop-blur-xl shadow-[0_16px_60px_rgba(0,0,0,0.22)]">
      ...
      <section className="mt-8 md:mt-10">
        ...
        <aside className="max-w-xl mx-auto rounded-2xl border border-white/15 bg-white/70 backdrop-blur-xl p-5 md:p-7 shadow-[0_12px_40px_rgba(0,0,0,0.14)] flex flex-col gap-4 md:gap-5">
          ...
          <div className="flex flex-col gap-2 mt-1">
            ...
            {hasOwnGroup && (
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-800 text-xs px-3 py-2">
                Vous avez déjà créé un groupe pour cet événement.
              </div>
            )}
          </div>
      {allGroups.length === 0 ? (
        ...
        {allGroups.slice(0, 5).map((g, i) => (
                <div
                  ...
                  className={`group relative w-12 h-12 rounded-full -ml-4 first:ml-0 ring-2 ring-white/80 shadow transition-transform duration-150 hover:scale-105 hover:z-50`}
                  style={{ zIndex: 50 - i }}
                >
                  {g.avatarUrl ? (
                    <img
        ...
        {allGroups.length > 5 && (
                <div className="-ml-4 w-12 h-12 rounded-full bg-slate-900 text-white flex items-center justify-center text-[11px] font-bold ring-2 ring-white/80 shadow">
          +{allGroups.length - 5}
                </div>
              )}
            </div>
        ...
        <article className="mt-6 md:mt-8 max-w-3xl mx-auto rounded-2xl border border-white/15 bg-white/70 backdrop-blur-xl p-5 md:p-7 shadow-[0_12px_40px_rgba(0,0,0,0.14)]">
          ...
          {(() => {
            ...
            return (
              <div>
                <p className={"text-[15px] leading-7 text-slate-800 whitespace-pre-line " + (!showFullDesc ? "line-clamp-4" : "")}>{text}</p>
                {isLong && (
                  <button
                    type="button"
                    onClick={() => setShowFullDesc((v) => !v)}
                    className="mt-2 text-sm font-semibold text-slate-900 underline underline-offset-2"
                  >

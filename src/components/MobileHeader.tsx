  ...
  return (
    <div className="md:hidden">
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[min(560px,calc(100%-2rem))]">
        <div className="backdrop-blur-xl bg-white/70 dark:bg-white/5 border border-black/10 dark:border-white/15 shadow-lg rounded-2xl">
          <div className="px-2 py-2 h-12 flex items-center gap-2">
            <button
              type="button"
              onClick={() => router.back()}
              aria-label="Retour"
              className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-900 dark:text-white hover:bg-black/5 dark:hover:bg-white/10"
            >
            ...
            {dynTitle || title ? (
              <div className="flex-1 truncate font-medium text-slate-900 dark:text-white">{dynTitle || title}</div>
            ) : (
